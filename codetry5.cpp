...
int main() {
  ...
  /*********** STEP 1: Preparation. Collect the information for the boolean
  expression***********/
  ...
  /***********STEP 1.B: Transform the decimal indices into Binary format and obtain relative
  information.***********/
...
grouped with other terms */
  ...
  for (i = 0; i < NumberOfVariable + 1; i++) {
    for (j = 0; j < NumberOfVariable + 1 - i; j++) {
      Column[i][j] = (int **
      ) malloc(Combination(NumberOfVariable, i, j) * sizeof(int * ));
      /* Column[i][j]
